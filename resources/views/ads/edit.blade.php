...
@section('content')
	...
	<div>
		<div style="max-width: 500px;" class="mx-auto">
			...
			<form action="{{ route('ads.update', $model->id) }}" method="POST" enctype="multipart/form-data">
				@csrf
				@method('PUT')
				<div class="mt-3">
				...
				<div class="mt-3">
					<div class="form-select">
						<select name="status" id="Status" placeholder="Status" onfocus="this.placeholder = ''" onblur="this.placeholder = 'Status'" required>
							@foreach(App\Models\Ads::$statuses as $st)
								<option value="{{ $st['value'] }}" {{ $st['value'] == $model->status ? 'selected' : '' }}>{{ $st['text'] }}</option>
							@endforeach
						</select>
					</div>
				</div>
