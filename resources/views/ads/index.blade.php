...
@section('content')
	<div>
		...
		<div class="button-group-area mb-3">
			<a href="{{ route('ads.create') }}" class="genric-btn primary radius">Create</a>
			<a href="{{ route('ads.index') }}" class="genric-btn info radius">Сбросить</a>
		</div>
			...
			<table class="my-table">
				<thead class="">
					<tr>
						<th class="pl-4">#</th>
						<th class="pl-4">
							<a href="{{ route('ads.index', ['sort' => 'title', 'order' => ($sort === 'title' && $order === 'ASC') ? 0 : 1]) }}">Title 
								@if($sort === 'title')
						...
						<th class="pl-4">
							<a href="{{ route('ads.index', ['sort' => 'year', 'order' => ($sort === 'year' && $order === 'ASC') ? 0 : 1]) }}">Year 
								@if($sort === 'year')
									<i class="fa fa-long-arrow-{{ $order === 'ASC' ? 'down' : 'up' }}" aria-hidden="true"></i>
								@endif
							</a>
						...
						<th class="pl-4">
							<a href="{{ route('ads.index', ['sort' => 'status', 'order' => ($sort === 'status' && $order === 'ASC') ? 0 : 1]) }}">Status 
								@if($sort === 'status')
									<i class="fa fa-long-arrow-{{ $order === 'ASC' ? 'down' : 'up' }}" aria-hidden="true"></i>
								@endif
							</a>
						</th>
						<th class="pl-4"> <span>Owner</span> </th>
						<th class="pl-4"></th>
					</tr>
				</thead>
				<tbody>
					@foreach($models as $key => $model)
						<tr>
							...
							<td class="pl-4">{{ $model->owner ? $model->owner->email : '' }}</td>
							<td class="pl-4 text-right">
								<div class="mb-2">
									<a href="{{ route('ads.show', $model->id) }}" class="genric-btn info-border radius">View</a>
								</div>
								<div class="mb-2">
									<a href="{{ route('ads.edit', $model->id) }}" class="genric-btn primary-border radius">Edit</a>
								</div>
								<div>
									<form action="{{ route('ads.destroy', $model->id) }}" method="POST">
										@csrf()
										@method('DELETE')
