...
@section('content')
	<div>
		<h3 class="mb-30 title_color">Cities</h3>
		<div class="button-group-area">
			<a href="{{ route('cities.create') }}" class="genric-btn primary radius">Create</a>
			<a href="{{ route('cities.index') }}" class="genric-btn info radius">Сбросить</a>
		</div>
			...
			<table class="my-table">
				<thead class="">
					<tr>
						<th class="pl-4">#</th>
						<th class="pl-4">
							<a href="{{ route('cities.index', ['sort' => 'title', 'order' => ($sort === 'title' && $order === 'ASC') ? 0 : 1]) }}">Title 
								@if($sort === 'title')
						...
						<th class="pl-4">
							<a href="{{ route('cities.index', ['sort' => 'sub_title', 'order' => ($sort === 'sub_title' && $order === 'ASC') ? 0 : 1]) }}">Sub Title 
								@if($sort === 'sub_title')
									<i class="fa fa-long-arrow-{{ $order === 'ASC' ? 'down' : 'up' }}" aria-hidden="true"></i>
								@endif
							</a>
						...
						<tr>
							...
							<td class="pl-4 text-right">
								<div class="button-group-area mt-10">
									<a href="{{ route('cities.show', $model->id) }}" class="genric-btn info-border radius">View</a>
									<a href="{{ route('cities.edit', $model->id) }}" class="genric-btn primary-border radius">Edit</a>
									<div class="d-inline-block">
										<form action="{{ route('cities.destroy', $model->id) }}" method="POST">
											@csrf()
											@method('DELETE')
