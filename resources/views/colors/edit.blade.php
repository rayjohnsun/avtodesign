...
@section('content')
	...
	<div>
		<div style="max-width: 500px;" class="mx-auto">
			<h3>Update Color: {{ $model->title }}</h3>
			<form action="{{ route('colors.update', $model->id) }}" method="POST">
				@csrf
				@method('PUT')
				<div class="mt-3">
				...
				<div class="mt-3" style="position: relative;">
					<input type="text" name="color" placeholder="Color" onfocus="this.placeholder = ''" onblur="this.placeholder = 'Color'" required class="single-input" value="{{ $model->color }}" id="ColorInput">
					<div id="ColorInputDiv" class="color-box"></div>
				</div>
				<div class="mt-3">
...
@section('js-script')
	...
	<script>
		$(function() {
			...
			$('#ColorInputDiv').css('background-color', '{{ $model->color }}');
